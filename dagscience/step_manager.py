...
class Step():
    ...
    def step_get_data(self, execute):
        ...
        if not execute:
            self.logger.info('Step 1: Not getting new data, loading from file system directly')
            return self.data_class.load_from_filesystem()
        else:
            self.logger.info('Step 1: Loading data from original source and saving to filesystem')
    ...
    def step_train(self, data, execute):
        ...
        if not execute:
            self.logger.info('Step 3: Not using training, loading model from file system')
